...
# $16 (test_rounds): optional space-separated round counts for evaluation, e.g. "50" or "5 10 20 50"
...
python examples/train_nn.py --d "$1" --t "$2" --dt "$3" --batch_size "$4" --n_batches "$5" --n_epochs "$6" --p "$7" \
    ...
    ${12:+--test} ${13:+--auto_batch_size} ${14:+--p_list ${14}} ${15:+--noise_model "${15}"} ${16:+--test_rounds ${16}}
# sbatch run_training.sh  d  t  dt  batch  nbatch  epochs  p  [intermediate]  [note]  [load_path]  [wandb_project]  [test]  [autobatch]  [p_list]          [noise_model]  [test_rounds]
...
# sbatch run_training.sh  3  50  2  2048   256     200     0.001  ""   ""        ""  GNN-google-data          test  ""         "0.001 0.005"                  SI1000  50