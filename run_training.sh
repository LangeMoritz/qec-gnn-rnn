...
# $16 (hidden_size): optional GRU hidden size and final GNN output dim (default: 256)
# $17 (n_gru_layers):optional number of GRU layers (default: 4)
# $18 (gnn_width):   optional intermediate GNN layer width; embedding_features=[3, gnn_width, hidden_size] (default: 64)
python examples/train_nn.py --d "$1" --t "$2" --dt "$3" --batch_size "$4" --n_batches "$5" --n_epochs "$6" --p "$7" \
    ...
    ${16:+--hidden_size "${16}"} ${17:+--n_gru_layers "${17}"} ${18:+--gnn_width "${18}"}
# sbatch run_training.sh  d  t  dt  batch  nbatch  epochs  p  [intermediate]  [note]  [load_path]  [wandb_project]  [test]  [p_list]          [noise_model]  [test_rounds]  [hidden_size]  [n_gru_layers]  [gnn_width]
...
# sbatch run_training.sh  3  50  2  2048   256     200     0.001  ""   ""        ""  GNN-google-data          test  "0.001 0.005"  SI1000  ""  512  6  128