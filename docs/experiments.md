...
## Experiment 12: Hierarchical Decoder d=5 — Frozen vs. Trainable vs. Random GNN (`iterative-decoding` branch, 2026-02-27)

**Goal**: Ablate the contribution of pre-trained base GNN weights and gradient flow in the hierarchical `MetaGRUDecoder`. Three conditions: (A) pretrained+frozen base, (B) pretrained+trainable base, (C) random-init+trainable base.
...
| meta\_hidden | 256 |
| n\_meta\_layers | 4 |
| CNN | Conv2d(embed→H,k=2)+ReLU+Conv2d(H→H,k=1)+ReLU |
| GPU | A40 (Alvis) |

| Job | wandb ID | Note | GNN weights | GNN trainable | Status |
|-----|----------|------|-------------|---------------|--------|
| 6005309 | `zjc29y31` | `ctrl_frozen` | pretrained (Exp 9) | frozen | done |
| 6005310 | `8jonoyqe` | `trainable_gnn` | pretrained (Exp 9) | trainable | done |
| 6005311 | `3o2nrnfj` | `random_gnn` | random init | trainable | crashed (CUDA OOM in test) |
| 6018895 | `ybwowhv6` | `random_gnn` (test-only resubmit) | — | — | running |

**Training results** (avg over last 20 epochs, MWPM ≈ 94.86%):

| Condition | Final acc | MWPM gap | First beats MWPM |
|-----------|-----------|----------|------------------|
| Frozen GNN | 96.01% | +1.15% | epoch 390 |
| Random GNN | 96.43% | +1.55% | epoch 214 |
| Trainable GNN | 96.73% | +1.87% | epoch 166 |

**Observations**:
- **Trainable GNN is best overall**: highest final accuracy (+1.87% over MWPM) and fastest convergence (beats MWPM at epoch 166). Pretrained d=3 weights provide a useful initialisation and joint gradient flow fine-tunes both base and meta layers for d=5.
- **Random GNN beats Frozen GNN** (96.43% vs. 96.01%, MWPM-crossing at epoch 214 vs. 390): freezing the pretrained base is actively harmful. Locking in d=3 representations forces the meta-GRU to compensate for mismatched features. A freely trainable random-init GNN adapts without this constraint.
- **Trainable GNN shows catastrophic forgetting spikes** in epochs 63–134 (accuracy drops up to 14.7% in a single epoch, e.g. 0.945→0.798 at epoch 63, followed by rapid 1–3-epoch recovery). Mechanism: d=5 gradients propagating into the pretrained d=3 GNN occasionally land in a poor weight region, partially overwriting useful features. This does not occur in frozen (no updates) or random (no useful structure to destroy, max single-epoch drop 4.9%). Spikes cease after epoch ~200 once the joint minimum is found (late-stage std 0.0008 vs. early-stage 0.0507).
- **Mitigation**: use separate per-parameter-group learning rates (e.g. `1e-5` for base GNN, `1e-4` for meta layers) to allow joint optimisation without catastrophic forgetting.
