...
class BBGRUDecoder(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch_labels, label_map):
        ...
        _, h = self.rnn(bulk)
        # h[-1]: [B, hidden_size] — last-layer final hidden state
        final_prediction = self.decoder(h[-1])   # [B, k]
    ...
    def train_model(
        ...
        for i in range(epoch_offset + 1, epoch_offset + self.args.n_epochs + 1):
            ...
            for batch_or_idx in batch_iter:
                ...
                final_pred = self.forward(x, edge_index, edge_attr, batch_labels, label_map)
