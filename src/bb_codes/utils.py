...
class Graph():
    ...
    def run_dijkstra_for_all_nodes(self):
        ...
        for i in range(self.V):
            adj[i, :] = self.dijkstra(i)
        return adj

def get_connectivity_from_check_matrices(hz, hx, no_of_participations_in_checks):
    # mind the qubit numbering: first Z checks, then X checks:
    h = np.vstack((hz, hx))
    no_of_qubits = h.shape[1]
    no_of_checks = h.shape[0]
    first_index = []
    second_index = []
    for i in range(no_of_qubits):
        # find the common checks for each data qubit (row):
        common_checks = np.nonzero(h[:, i])
        # create edges between all the checks of each data qubit:
        first_index.extend(np.repeat(common_checks, no_of_participations_in_checks))
        second_index.extend(np.tile(common_checks, no_of_participations_in_checks)[0])
    # create an edge list from all edges:
    first_index = np.array(first_index)
    second_index = np.array(second_index)
    edge_list = np.vstack((first_index, second_index))

    connectivity = np.zeros((no_of_checks, no_of_checks))
    connectivity[edge_list[0], edge_list[1]] = 1

    # remove self edges:
    np.fill_diagonal(connectivity, 0)
    return connectivity

def get_adjacency_matr_from_check_matrices(hz, hx, no_of_participations_in_checks):
    connectivity = get_connectivity_from_check_matrices(hz, hx, no_of_participations_in_checks)
    no_nodes = connectivity.shape[0]
    g = Graph(no_nodes)
    g.graph = connectivity
    adjacency_matr = g.run_dijkstra_for_all_nodes()
